...
from markups import start_message, date_message, wrong_date_message, \
    need_correct_date_message, time_message, uncorrect_time_message, phone_message, url_buttons
...
async def cmd_start(message: types.Message):
    ...
    """
    await message.answer(start_message, reply_markup=url_buttons)
...
async def cancel_handler(message: types.Message, state: FSMContext):
    ...
    """
    await state.finish()
    await message.answer('Вы отменили свою запись')


# @dp.callback_query_handler(text='Записаться')
# async def reserve_handler(callback: types.CallbackQuery):
#     """
